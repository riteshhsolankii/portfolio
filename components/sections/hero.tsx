...
export default function Hero() {
  ...
  return (
    ...
    >
      <div className="relative z-10 mx-auto grid w-full max-w-[1440px] items-center gap-16 lg:grid-cols-[1.2fr_0.8fr]">
        <div>
          <motion.p
            initial={{ opacity: 0, y: 16 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 1.5, duration: 0.6 }}
            className="mb-5 inline-flex items-center gap-2 rounded-full border border-border bg-card px-4 py-1.5 text-sm text-foreground/80 backdrop-blur-xl"
          >
          ...
          <motion.div
            ...
            className="mt-6 flex items-center gap-3 font-heading text-lg text-foreground/90 sm:text-xl"
          >
        ...
        >
          <div className="glass relative aspect-[4/5] overflow-hidden rounded-[2.5rem]">
            ...
            <div className="absolute inset-x-0 bottom-0 bg-background/80 p-6">
              <p className="font-heading text-xl font-bold uppercase tracking-wide text-foreground">
                {SITE.name}
              </p>
          ...
          {floatingBadges.map(({ icon: Icon, className, label }, i) => (
            <div
              key={label}
              title={label}
              className={`absolute flex h-14 w-14 items-center justify-center rounded-2xl border border-border bg-background shadow-glass ${className} ${
      ...
      >
        <Link to="/#about" aria-label="Scroll to about section">
          <span className="flex h-10 w-6 items-start justify-center rounded-full border border-foreground/30 p-1.5">
